...
vector<VulkanMesh> VulkanMeshModel::loadNode(VkPhysicalDevice physicalDeviceP, VkDevice deviceP, VkQueue transferQueue, VkCommandPool transferCommandPool, aiNode* node, const aiScene* scene, vector<int> matToTex)
{
	...
	{
		// Load mesh
		meshes.push_back(loadMesh(physicalDeviceP, deviceP, transferQueue, transferCommandPool, scene->mMeshes[node->mMeshes[i]], scene, matToTex));
		// Explanation of scene->mMeshes[node->mMeshes[i]]:
		// The scene actually hold the data for the meshes, and the nodes store ids of meshes, that relate to the scene meshes.
	}
	...
	{
		vector<VulkanMesh> newMeshes = loadNode(physicalDeviceP, deviceP, transferQueue, transferCommandPool, node->mChildren[i], scene, matToTex);
...
VulkanMesh VulkanMeshModel::loadMesh(VkPhysicalDevice physicalDeviceP, VkDevice deviceP, VkQueue transferQueue, VkCommandPool transferCommandPool, aiMesh* mesh, const aiScene* scene, vector<int> matToTex)
{
	...
	// Create new mesh
	VulkanMesh newMesh = VulkanMesh(physicalDeviceP, deviceP, transferQueue, transferCommandPool, &vertices, &indices, matToTex[mesh->mMaterialIndex]);
	return newMesh;
}
