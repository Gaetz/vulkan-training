...
#include <../lib/SDL2-2.0.18/include/SDL.h>
#include <../lib/SDL2-2.0.18/include/SDL_vulkan.h>



static void check_result(VkResult result);
#define check(result) GASSERTM(result == VK_SUCCESS, "Vulkan assert code %u", result)
...
static FlatHashMap<u64, VkRenderPass> render_pass_cache;
...
bool GpuDevice::get_family_queue(VkPhysicalDevice physical_device)
{
    ...
    VkQueueFamilyProperties* queue_families = (VkQueueFamilyProperties*)GAlloca(sizeof(VkQueueFamilyProperties) * queue_family_count, allocator);
    ...
    GFree(queue_families, allocator);
...
void GpuDevice::init(const DeviceCreation& creation)
{
...
#ifdef VULKAN_DEBUG_REPORT
    {
        ...
        VkExtensionProperties* extensions = (VkExtensionProperties*)GAlloca(sizeof(VkExtensionProperties) * num_instance_extensions, allocator);
        ...
        GFree(extensions, allocator);

        if (!debug_utils_extension_present)
        {
            GPrint("Extension %s for debugging non present.", VK_EXT_DEBUG_UTILS_EXTENSION_NAME);
        }
        else
        {
            ...
            // GASSERT( vkCreateDebugReportCallbackEXT != NULL, "" );
    ...
    VkPhysicalDevice* gpus = (VkPhysicalDevice*)GAlloca(sizeof(VkPhysicalDevice) * num_physical_device, allocator);
    ...
    {
        GPrint("Failed to create Vulkan surface.\n");
    }
    ...
    {
        GASSERTM(false, "Suitable GPU device not found!");
        return;
    }

    GFree(gpus, allocator);
    ...
    VkSurfaceFormatKHR* supported_formats = (VkSurfaceFormatKHR*)GAlloca(sizeof(VkSurfaceFormatKHR) * supported_count, allocator);
    ...
    GFree(supported_formats, allocator);
    ...
    u8* memory = GAllocaM(sizeof(GPUTimestampManager) + sizeof(CommandBuffer*) * 128, allocator);
    ...
    GASSERTM(queued_command_buffers == correctly_allocated_buffer, "Wrong calculations for queued command buffers arrays. Should be %p, but it is %p.", correctly_allocated_buffer, queued_command_buffers);
    ...
    char* compiler_path = string_buffer.AppendUseF("%s\\Bin\\", vulkan_env);
#else
...
void GpuDevice::shutdown()
{
    ...
    GFree(gpu_timestamp_manager, allocator);
    ...
    FlatHashMapIterator it = render_pass_cache.IteratorBegin();
    while (it.isValid())
    {
        ...
        render_pass_cache.IteratorAdvance(it);
    }
...
{
    GPrint("Error in creation of shader %s, stage %s. Writing shader:\n", name, to_stage_defines(stage));
    ...
    while (current_code)
    {
        ...
        char* line = temp_string_buffer.AppendUseSubstring(current_code, 0, (end_of_line - current_code));
        GPrint("%u: %s", line_index++, line);
...
VkShaderModuleCreateInfo GpuDevice::compile_shader(cstring code, u32 code_size, VkShaderStageFlagBits stage, cstring name)
{
    ...
    temp_string_buffer.Init(GKilo(1), temporary_allocator);

    // Add uppercase define as STAGE_NAME
    char* stage_define = temp_string_buffer.AppendUseF("%s_%s", to_stage_defines(stage), name);
    ...
    // Compile to SPV
#if defined(_MSC_VER)
    char* glsl_compiler_path = temp_string_buffer.AppendUseF("%sglslangValidator.exe", vulkan_binaries_path);
    char* final_spirv_filename = temp_string_buffer.AppendUse("shader_final.spv");
    // TODO: add optional debug information in shaders (option -g).
    char* arguments = temp_string_buffer.AppendUseF("glslangValidator.exe %s -V --target-env vulkan1.2 -o %s -S %s --D %s --D %s", temp_filename, final_spirv_filename, to_compiler_extension(stage), stage_define, to_stage_defines(stage));
#else
...
#endif
    ProcessExecute(".", glsl_compiler_path, arguments, "");
    ...
    if (optimize_shaders)
    {
        ...
        char* spirv_optimizer_path = temp_string_buffer.AppendUseF("%sspirv-opt.exe", vulkan_binaries_path);
        char* optimized_spirv_filename = temp_string_buffer.AppendUseF("shader_opt.spv");
        char* spirv_opt_arguments = temp_string_buffer.AppendUseF("spirv-opt.exe -O --preserve-bindings %s -o %s", final_spirv_filename, optimized_spirv_filename);
    ...
    FileDelete(final_spirv_filename);
...
ShaderStateHandle GpuDevice::create_shader_state(const ShaderStateCreation& creation)
{
    ...
    {
        GPrint("Shader %s does not contain shader stages.\n", creation.name);
        return handle;
    }
    ...
    for (compiled_shaders = 0; compiled_shaders < creation.stages_count; ++compiled_shaders)
    {
        ...
        // if ( compiled ) {
        //    GFree( ( void* )createInfo.pCode, allocator );
        //}
    ...
    if (creation_failed)
    {
        ...
        // Dump shader code
        GPrint("Error in creation of shader %s. Dumping all shader informations.\n", creation.name);
        for (compiled_shaders = 0; compiled_shaders < creation.stages_count; ++compiled_shaders)
        {
            const ShaderStage& stage = creation.stages[compiled_shaders];
...
DescriptorSetLayoutHandle GpuDevice::create_descriptor_set_layout(const DescriptorSetLayoutCreation& creation)
{
    ...
    u8* memory = GAllocaM((sizeof(VkDescriptorSetLayoutBinding) + sizeof(DescriptorBinding)) * creation.num_bindings, allocator);
    ...
    for (u32 r = 0; r < creation.num_bindings; ++r)
    {
        ...
        binding.start = input_binding.start == u16Max ? (u16)r : input_binding.start;
        binding.count = 1;
    ...
    VkSampler vk_default_sampler, u32& num_resources, const ResourceHandle* resources, const SamplerHandle* samplers, const u16* bindings)
{
    ...
    for (u32 r = 0; r < num_resources; r++)
    {
        ...
        default:
        {
            GASSERTM(false, "Resource type %d not supported in descriptor set creation!\n", binding.type);
            break;
        }
        }
    }
...
DescriptorSetHandle GpuDevice::create_descriptor_set(const DescriptorSetCreation& creation)
{
    ...
    // Cache data
    u8* memory = GAllocaM((sizeof(ResourceHandle) + sizeof(SamplerHandle) + sizeof(u16)) * creation.num_resources, allocator);
...
void GpuDevice::destroy_buffer(BufferHandle buffer)
{
    ...
    {
        GPrint("Graphics error: trying to free invalid Buffer %u\n", buffer.index);
    }
}
...
void GpuDevice::destroy_texture(TextureHandle texture)
{
    ...
    {
        GPrint("Graphics error: trying to free invalid Texture %u\n", texture.index);
    }
}
...
void GpuDevice::destroy_pipeline(PipelineHandle pipeline)
{
    ...
    {
        GPrint("Graphics error: trying to free invalid Pipeline %u\n", pipeline.index);
    }
}
...
void GpuDevice::destroy_shader_state(ShaderStateHandle shader)
{
    ...
    {
        GPrint("Graphics error: trying to free invalid Shader %u\n", shader.index);
    }
}
...
void GpuDevice::destroy_descriptor_set_layout_instant(ResourceHandle descriptor_set_layout)
{
    ...
    if (v_descriptor_set_layout)
    {
        ...
        GFree(v_descriptor_set_layout->bindings, allocator);
    }
...
void GpuDevice::destroy_descriptor_set_instant(ResourceHandle descriptor_set)
{
    ...
    if (v_descriptor_set)
    {
        ...
        GFree(v_descriptor_set->resources, allocator);
...
void GpuDevice::create_swapchain()
{
    ...
    {
        GPrint("Error no WSI support on physical device 0\n");
    }
...
void GpuDevice::resize_swapchain()
{
    ...
    // GPrint( "Requested swapchain resize %u %u\n", swapchain_extent.width, swapchain_extent.height );
    if (swapchain_extent.width == 0 || swapchain_extent.height == 0)
    {
        // GPrint( "Cannot create a zero-sized swapchain\n" );
        return;
    }
...
void GpuDevice::update_descriptor_set(DescriptorSetHandle descriptor_set)
{
    ...
    {
        GPrint("Graphics error: trying to update invalid DescriptorSet %u\n", descriptor_set.index);
    }
}
    ...
    if (descriptor_set_updates.size)
    {
        for (i32 i = descriptor_set_updates.size - 1; i >= 0; i--)
        {
            ...
            // if ( update.frame_issued == current_frame )
            {
                ...
                update.frame_issued = u32Max;
                descriptor_set_updates.DeleteSwap(i);
            }
        }
    }
}
...
void GpuDevice::present()
{
    ...
    if (timestamps_enabled)
    {
        ...
        {
            GPrint("Asymmetrical GPU queries, missing pop of some markers!\n");
        }
    ...
    if (resource_deletion_queue.size > 0)
    {
        for (i32 i = resource_deletion_queue.size - 1; i >= 0; i--)
        {
            ...
            if (resource_deletion.current_frame == current_frame)
            {
                ...
                resource_deletion.current_frame = u32Max;

                // Swap element
                resource_deletion_queue.DeleteSwap(i);
            }
        }
    }
}
...
void* GpuDevice::dynamic_allocate(u32 size)
{
    ...
    dynamic_allocated_size += (u32)MemoryAlign(size, s_ubo_alignment);
...
void check_result(VkResult result)
{
    ...
    GPrint("Vulkan error: code(%u)", result);
    if (result < 0)
    {
        GASSERTM(false, "Vulkan error: aborting.");
    }
}
// Device ////////////////////////////////////////////////////////////////
...
void GPUTimestampManager::init(Allocator* allocator_, u16 queries_per_frame_, u16 max_frames)
{
    ...
    u8* memory = GAllocaM(allocated_size, allocator);
