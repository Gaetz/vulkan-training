...
#include <stdio.h>
    ...
    FlatHashMap<ResourceHandle, ResourceHandle> g_texture_to_descriptor_set;
    ...
    void ImGuiService::Shutdown() {
        ...
        FlatHashMapIterator it = g_texture_to_descriptor_set.IteratorBegin();
        while (it.isValid()) {
            ResourceHandle handle = g_texture_to_descriptor_set.Get(it);
            gpu->destroy_descriptor_set({ handle });

            g_texture_to_descriptor_set.IteratorAdvance(it);
        }
    ...
    void ImGuiService::render(CommandBuffer& commands) {
        ...
        if (vertex_size >= g_vb_size || index_size >= g_ib_size) {
            GPrint("ImGui Backend Error: vertex/index overflow!\n");
            return;
        }
    ...
    void ImGuiService::remove_cached_texture(TextureHandle& texture) {
        FlatHashMapIterator it = g_texture_to_descriptor_set.find(texture.index);
        if (it.isValid()) {

            // Destroy descriptor set
            DescriptorSetHandle descriptor_set{ g_texture_to_descriptor_set.Get(it) };
    ...
    static g::StringArray           files;
    static g::StringArray           directories;
    ...
    bool imgui_file_dialog_open( const char* button_name, const char* path, const char* extension ) {
        ...
        if ( opened && ImGui::Begin( "raptor_imgui_file_dialog", &opened, ImGuiWindowFlags_AlwaysAutoResize ) ) {
            ...
            g::MemoryAllocator* allocator = g::memory_get_system_allocator();
            ...
            if ( strcmp( path, last_path ) != 0 ) {
                ...
                g::file_open_directory( path, &directory );
            ...
            if ( scan_folder ) {
                ...
                g::file_find_files_in_path( extension, directory.path, files, directories );
            }
            ...
            for ( size_t d = 0; d < directories.get_string_count(); ++d ) {
                ...
                if ( ImGui::Selectable( directory_name, selected, ImGuiSelectableFlags_AllowDoubleClick ) ) {
                    ...
                    if ( strcmp( directory_name, ".." ) == 0 ) {
                        g::file_parent_directory( &directory );
                    } else {
                        g::file_sub_directory( &directory, directory_name );
                    }
    ...
    bool imgui_path_dialog_open( const char* button_name, const char* path ) {
        ...
        if ( opened && ImGui::Begin( "raptor_imgui_file_dialog", &opened, ImGuiWindowFlags_AlwaysAutoResize ) ) {
            ...
            if ( scan_folder ) {
                ...
                g::file_find_files_in_path( ".", directory.path, files, directories );
            }
