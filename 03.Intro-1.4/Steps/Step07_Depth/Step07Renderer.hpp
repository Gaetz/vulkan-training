...
// Step 07 — Depth Buffering: depth image/view, PipelineDepthStencilState,
// depth attachment in dynamic rendering. Two overlapping squares at z=0 / z=-0.5.
class Step07Renderer : public IRenderer {
public:
    // -------------------------------------------------------------------------
    // Vertex layout — position is now float3 (z depth)
    // -------------------------------------------------------------------------
    struct Vertex {
        glm::vec3 pos;
        glm::vec3 color;
...
private:
    ...
    // --- Texture (image + view + sampler; destroyed before VMA allocator) ---
    Texture texture;
