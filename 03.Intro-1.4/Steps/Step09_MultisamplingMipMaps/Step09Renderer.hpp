...
// Step 09 — Multisampling: adds MSAA to the Step 08 renderer.
// A multi-sampled color image is used as the render target; the swapchain image
// is the resolve target (specified via resolveImageView in RenderingAttachmentInfo).
// Depth is also multi-sampled. Sample count is chosen at runtime based on
// device capabilities.
class Step09Renderer : public IRenderer {
public:
    // -------------------------------------------------------------------------
    // Vertex layout — same as Step 08 (float3 pos, float3 color, float2 uv).
    // -------------------------------------------------------------------------
    struct Vertex {
        glm::vec3 pos;
        glm::vec3 color;
    ...
    ~Step09Renderer() override;

    void waitIdle() override { if (*device) device.waitIdle(); }

    // Called by the Engine after scene->init() — lets the renderer bind
    // scene resources (texture) to its descriptor sets.
    void onSceneReady(IScene& scene) override;

    // Scene-facing loading API — called from Step09Scene::init()
...
private:
    ...
    // --- MSAA color image (multi-sampled render target, swapchain-sized) ---
